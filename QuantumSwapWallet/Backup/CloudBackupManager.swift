...
// Android reference:
// app/src/main/java/com/quantumswap/app/backup/CloudBackupManager.java
...
public final class CloudBackupManager: NSObject {
    ...
    /// therefore validates `address` against `QuantumSwapAddress.isValid`
    ...
    /// pre-validate (`guard QuantumSwapAddress.isValid(addr) else { ... }`)
    ...
    public static func buildFilename(address: String) -> String {
        ...
        let hex: String
        if QuantumSwapAddress.isValid(address) {
    ...
    /// `QuantumSwapAddress.isValid` BEFORE being returned, so any
    ...
    public static func extractAddress(fromEncryptedJson json: String) -> String? {
        ...
        return QuantumSwapAddress.normalized(prefixed)
    }
    ...
    public func exportWalletFile(address: String, walletJson: String, from vc: UIViewController) {
        ...
        // that fails `QuantumSwapAddress.isValid`. We re-validate here
        ...
        guard QuantumSwapAddress.isValid(address) else {
